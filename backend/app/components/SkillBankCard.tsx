    ...
    <div className="w-full bg-slate-50/90 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-4 sm:p-6 backdrop-blur-xl transition-all shadow-xl">
      ...
      {!skillBank ? (
        ...
        <div className="space-y-5 sm:space-y-6">
          ...
          {activeCategory !== "Projects" ? (
              ...
              {skillBank.skills
                ...
                .map((item, idx) => (
                  <div
                    ...
                    className="group relative bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:border-purple-500/40 px-2.5 sm:px-3 py-1.5 rounded-xl flex flex-wrap items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs text-slate-800 dark:text-slate-200 transition-all hover:bg-purple-50 dark:hover:bg-purple-500/10 cursor-help shadow-sm"
                  >
                    <span className="font-medium">{item.name}</span>
                    {item.versionDetails && (
                      <span className="text-[9px] sm:text-[10px] bg-purple-500/10 dark:bg-purple-500/20 text-purple-700 dark:text-purple-300 px-1.5 py-0.5 rounded border border-purple-500/30 font-mono">
                        {item.versionDetails}
                      </span>
                    )}
                    <span className="text-[9px] sm:text-[10px] text-purple-700 dark:text-purple-300/80 group-hover:text-purple-800 dark:group-hover:text-purple-200 bg-purple-500/10 px-1.5 py-0.5 rounded border border-purple-500/20 truncate max-w-[140px] sm:max-w-[200px]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-72 overflow-y-auto pr-1 sm:pr-2 custom-scrollbar">
              {(skillBank.projects || []).map((proj, idx) => (
                <div key={idx} className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-3 flex flex-col justify-between gap-2 shadow-sm">
                  <div>
                    <div className="flex items-center justify-between gap-2">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-xs font-bold text-indigo-600 dark:text-indigo-300 hover:text-indigo-700 dark:hover:text-indigo-200 hover:underline flex items-center gap-1.5 truncate"
                      >
                      ...
                      <span className="text-[10px] text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-white/5 px-2 py-0.5 rounded border border-slate-200 dark:border-white/10 shrink-0">
                        {proj.primaryLanguage}
                      </span>
                    </div>
                    <p className="text-[11px] text-slate-600 dark:text-slate-400 mt-1 line-clamp-2">{proj.description}</p>
                  </div>
                  {proj.extractedSkills && proj.extractedSkills.length > 0 && (
                    <div className="pt-2 border-t border-slate-200 dark:border-white/5">
                      <span className="text-[10px] text-purple-700 dark:text-purple-300/80 block mb-1 font-semibold">Extracted Skills:</span>
                      <div className="flex flex-wrap gap-1">
                        {proj.extractedSkills.map((sk, sIdx) => (
                          <span key={sIdx} className="text-[9px] bg-purple-500/10 text-purple-700 dark:text-purple-300 border border-purple-500/20 px-1.5 py-0.5 rounded font-medium">
                            {sk}
                          </span>
                        ))}
                      </div>
          ...
          {matchResult && (
            <div className="mt-4 p-3.5 sm:p-4 rounded-xl bg-purple-500/5 border border-purple-500/20 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-semibold text-purple-700 dark:text-purple-300">JD vs. Skill Bank Match Matrix</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-600 dark:text-slate-400">ATS Match Index:</span>
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-md ${
                    matchResult.matchScore >= 75 ? "bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30" : "bg-amber-500/15 text-amber-700 dark:text-amber-300 border border-amber-500/30"
                  }`}>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {matchResult.matchedKeywords.map((m, i) => (
                      <span key={i} className="text-[10px] sm:text-[11px] bg-emerald-500/10 border border-emerald-500/20 text-emerald-700 dark:text-emerald-300 px-2 py-0.5 rounded-lg flex items-center gap-1 font-medium">
                        <span>{m.skill}</span>
                        {m.sourceRepo && (
                          <span className="text-[9px] opacity-80 bg-emerald-500/20 text-emerald-800 dark:text-emerald-200 px-1 py-0.2 rounded truncate max-w-[100px]">
                            {m.sourceRepo}
                          </span>
                        )}
                      </span>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {matchResult.missingGaps.length > 0 ? (
                      matchResult.missingGaps.map((gap, i) => (
                        <span key={i} className="text-[10px] sm:text-[11px] bg-rose-500/10 border border-rose-500/20 text-rose-700 dark:text-rose-300 px-2 py-0.5 rounded-lg font-medium">
                          {gap}
                        </span>
                      ))
                    ) : (
                ...
                <div className="pt-3 border-t border-purple-500/20 space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {matchResult.relevantProjects.map((p, pIdx) => (
                      <div key={pIdx} className="bg-white dark:bg-white/5 border border-purple-500/20 rounded-lg p-2.5 text-xs space-y-1 shadow-sm">
                        <div className="flex items-center justify-between font-bold text-purple-700 dark:text-purple-200 gap-2">
                          <a href={p.repoUrl} target="_blank" rel="noreferrer" className="hover:underline flex items-center gap-1 truncate">
                            <span className="truncate">{p.name}</span>
                          </a>
                          <span className="text-[9px] bg-purple-500/10 dark:bg-purple-500/20 px-1.5 py-0.5 rounded text-purple-700 dark:text-purple-300 shrink-0 font-medium">{p.primaryLanguage}</span>
                        </div>
                        <p className="text-[10px] text-slate-600 dark:text-slate-400 line-clamp-1">{p.description}</p>
                        {p.extractedSkills && p.extractedSkills.length > 0 && (
                          <div className="flex flex-wrap gap-1 pt-1">
                            {p.extractedSkills.slice(0, 6).map((sk, skIdx) => (
                              <span key={skIdx} className="text-[8px] bg-indigo-500/10 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 px-1.5 py-0.2 rounded border border-indigo-500/20 font-medium">
                                {sk}
                              </span>
                            ))}
                          </div>
